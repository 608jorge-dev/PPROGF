...
int main(int argc, char **argv)
{
  ...
  if (all || test == 28)
    test1_space_find_character();
  if (all || test == 29)
    test2_space_find_character();
  if (all || test == 30)
    test3_space_find_character();
  if (all || test == 31)
...
void test1_space_set_id()
{
  Space *s;
  s = space_create(5);
  PRINT_TEST_RESULT(space_set_id(s, 10) == OK);
  space_destroy(s);
}
...
{
  Space *s = NULL;
  PRINT_TEST_RESULT(space_set_id(s, 10) == ERROR);
}
...
void test1_space_set_gdesc()
{
  Space *s;
  s = space_create(5);
  PRINT_TEST_RESULT(space_set_gdesc(s, 1, "hola") == OK);
  space_destroy(s);
}
...
{
  Space *s = NULL;
  PRINT_TEST_RESULT(space_set_gdesc(s, 1, "hola") == ERROR);
}
...
void test3_space_set_gdesc()
{
  Space *s;
  s = space_create(5);
  PRINT_TEST_RESULT(space_set_gdesc(s, 1, NULL) == ERROR);
  space_destroy(s);
}
...
void test1_space_add_object()
{
  Space *s;
  s = space_create(5);
  PRINT_TEST_RESULT(space_add_object(s, 6) == OK);
  space_destroy(s);
}
...
void test1_space_find_object()
{
  ...
  s = space_create(5);
  space_add_object(s, 6);
  PRINT_TEST_RESULT(space_find_object(s, 6) == TRUE);
  space_destroy(s);
}
...
{
  Space *s = NULL;
  PRINT_TEST_RESULT(space_find_object(s, 6) == FALSE);
}
...
void test1_space_find_character()
{
  Space *s;
  s = space_create(5);
  space_set_character(s, 6);
  PRINT_TEST_RESULT(space_find_character(s, 6) == TRUE);
  space_destroy(s);
}
...
{
  Space *s = NULL;
  PRINT_TEST_RESULT(space_find_character(s, 6) == FALSE);
}
